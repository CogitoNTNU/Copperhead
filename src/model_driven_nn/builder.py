...
class Builder:
    def build(self, spec: YamlMapping, input_dim: int) -> BuildResult:
    ...
    @staticmethod
    def generic_build(spec: YamlMapping, _input_dim: int) -> Never:
        component_type = spec.get("type")
    ...
    @staticmethod
    def build_relu(_spec: YamlMapping, input_dim: int) -> tuple[ReLU, int]:
        return ReLU(), input_dim

    @staticmethod
    def build_linear(spec: YamlMapping, input_dim: int) -> tuple[Linear, int]:
        output_dim = positive_int(spec.get("output_dim"), "Linear output_dim")
        return Linear(input_dim, output_dim), output_dim

    def build_sequential(
        self, spec: YamlMapping, input_dim: int
    ...
    def build_concat(self, spec: YamlMapping, input_dim: int) -> tuple[Concat, int]:
    ...
    @staticmethod
    def _children(spec: YamlMapping, key: str) -> list[YamlMapping]:
        value = spec.get(key)
        if not isinstance(value, list) or not value:
            raise ValueError(f"'{key}' must be a nonempty list")

        children: list[YamlMapping] = []
